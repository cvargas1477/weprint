...
//funcion cargar Data
	function loadData(){		
		$('#consulta').DataTable({
			...
			order:[[0,"desc"]],
			ajax:{				
				url:'{{ route('aceptadocliente.index') }}',
				type:'GET',
				data:{
...
//funcion cargar Data detalle taller
	...
	$('#midetalletaller').DataTable({
			...
			order:[[0,"desc"]],
			ajax:{				
				url:'{{ route('aceptadocliente.show','1') }}',									
				type:'GET',
				data:{}
			},
			columns:[
...
$(document).on('click','.btn-detalletaller',function(){
	...
	url = '{{ route('aceptadocliente.show',':ventasid' ) }}';
...
$(document).on('click','.btn-edit',function(){
	...
	url = '{{ route('aceptadocliente.edit',':id' ) }}';
...
//Cambiar proceso taller
	$(document).on('submit','#agregar',function(e){
		...
		$.ajax({
			...
			url:'{{ route('aceptadocliente.store') }}',
			type:'POST',
			data:parametros,
			dataType:'JSON',
			beforeSend:function(){
